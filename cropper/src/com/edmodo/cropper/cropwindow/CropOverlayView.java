...
public class CropOverlayView extends View {
	...
	// The Xfermode used to mask crop area.
	private static final Xfermode mMode = new PorterDuffXfermode(
			PorterDuff.Mode.DST_IN);

	// The Bitmap used to mask crop area.
	...
	protected void onDraw(Canvas canvas) {
		...
		case OVAL:
			...
			break;
		case MASK:
			drawMaskBackground(canvas, mBitmapRect);
			break;
		default:
			break;
		}
		...
		case RECT:
			...
			canvas.drawRect(Edge.LEFT.getCoordinate(),
					Edge.TOP.getCoordinate(), Edge.RIGHT.getCoordinate(),
					Edge.BOTTOM.getCoordinate(), mBorderPaint);
			break;
		case OVAL:
			oval.set(Edge.LEFT.getCoordinate(), Edge.TOP.getCoordinate(),
		...
		if (mFixAspectRatio) {
			...
			if (AspectRatioUtil.calculateAspectRatio(bitmapRect) > mTargetAspectRatio) {
				...
				if (cropWidth == Edge.MIN_CROP_LENGTH_PX) {
					float aspectRatio = (Edge.MIN_CROP_LENGTH_PX)
				...
				if (cropHeight == Edge.MIN_CROP_LENGTH_PX) {
					float aspectRatio = (Edge.RIGHT.getCoordinate() - Edge.LEFT
	...
	private void drawOvalBackground(Canvas canvas, Rect bitmapRect) {
		...
		Path path = new Path();
		path.addRect(bitmapRect.left, bitmapRect.top, bitmapRect.right,
				bitmapRect.bottom, Path.Direction.CW);
		path.addOval(r, Path.Direction.CCW);
		path.setFillType(Path.FillType.EVEN_ODD);
		canvas.drawPath(path, mBackgroundPaint);
	}

	private void drawMaskBackground(Canvas canvas, Rect bitmapRect) {
		...
		int sc = canvas.saveLayer(bitmapRect.left, bitmapRect.top,
				bitmapRect.right, bitmapRect.bottom, null,
				Canvas.MATRIX_SAVE_FLAG | Canvas.CLIP_SAVE_FLAG
						| Canvas.HAS_ALPHA_LAYER_SAVE_FLAG
						| Canvas.FULL_COLOR_LAYER_SAVE_FLAG
						| Canvas.CLIP_TO_LAYER_SAVE_FLAG);

		canvas.drawRect(bitmapRect.left, bitmapRect.top, bitmapRect.right,
				bitmapRect.bottom, mBackgroundPaint);
		mBackgroundPaint.setXfermode(mMode);
		canvas.drawBitmap(mMaskBitmap, new Rect(0, 0, mMaskBitmap.getWidth(),
				mMaskBitmap.getHeight()), r, mBackgroundPaint);
